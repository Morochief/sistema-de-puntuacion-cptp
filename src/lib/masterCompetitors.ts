...
export async function renderMasterCompetitorsModal(onSelectCallback?: (mc: MasterCompetitor) => void): Promise<void> {
  ...
  // ── Renderizar estructura estatica UNA SOLA VEZ ─────────────────
  modalBox.innerHTML = `
    <div style="padding:16px 20px;border-bottom:1px solid #e2e8f0;background:#f8fafc;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
      <div>
        ...
        <span id="mc-total-count" style="font-size:0.75rem;color:#64748b;font-weight:600;">Base de datos de tiradores registrados</span>
      </div>
    ...
    <div style="padding:16px 20px;background:#ffffff;display:flex;flex-direction:column;gap:12px;">
      ...
      <!-- Tabla -->
      <div id="mc-table-wrapper" style="max-height:300px;overflow-y:auto;border:1px solid #e2e8f0;border-radius:8px;">
      ...
      <!-- Paginador -->
      <div id="mc-paginator" style="display:flex;justify-content:space-between;align-items:center;padding-top:4px;font-size:0.85rem;color:#64748b;"></div>
    </div>
  `;

  // ── Funcion que solo actualiza el tbody y el paginador ─────
  const renderTable = async () => {
    ...
    const tbody = modalBox.querySelector('#mc-table-body');
    if (!tbody) return;
    ...
    if (pageItems.length > 0) {
      ...
      tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;padding:24px;color:#94a3b8;">No se encontraron competidores en el Padrón.</td></tr>`;
    }

    const totalSpan = modalBox.querySelector('#mc-total-count');
    if (totalSpan) totalSpan.textContent = `Base de datos de tiradores registrados (${totalItems} en total)`;

    const paginator = modalBox.querySelector('#mc-paginator');
    if (paginator) {
      paginator.innerHTML = `
  ...
  // ── Event listeners (se atan una sola vez) ────────────────
  modalBox.querySelector('#close-mc-modal')?.addEventListener('click', () => backdrop.remove());
  ...
  modalBox.querySelector('#btn-sync-padron')?.addEventListener('click', async () => {
    ...
    try {
      ...
      showToast(added > 0 ? `${added} tiradores agregados al Padron Maestro.` : 'El Padron ya esta al dia.', added > 0 ? 'success' : 'info');
      await renderTable();
    } catch (err) {
  ...
  // ── Event listeners (se atan una sola vez, usan delegacion) ──
  ...
  // Delegacion de eventos via el modalBox (no se pierde al recrear el tbody/paginator)
  modalBox.addEventListener('click', async (e) => {
    const target = e.target as HTMLElement;
    if (target.id === 'mc-prev-page' && currentPage > 1) {
      currentPage--;
      await renderTable();
    } else if (target.id === 'mc-next-page') {
      currentPage++;
      await renderTable();
    } else if (target.matches('[data-delete-mc]')) {
      const id = Number(target.dataset.deleteMc);
    ...
    } else if (target.matches('[data-edit-mc]')) {
      const id = Number(target.dataset.editMc);
      // Buscar el competidor desde la DB directamente en vez de usar variable 'all' fuera de scope
      const comp = await db.masterCompetitors.get(id);
      if (!comp) return;
    ...
    } else if (target.matches('[data-select-mc]')) {
      const id = Number(target.dataset.selectMc);
      const comp = await db.masterCompetitors.get(id);
      if (comp && onSelectCallback) {
