const fs = require('fs');
let content = fs.readFileSync('src/lib/heatsManager.ts', 'utf8');

const startMarker = 'export async function showManualHeatsReorderModal';
const endMarker = '\nexport function applySharedRifleRules';
...
const newFunction = `export async function showManualHeatsReorderModal(eventId: number, onSaveCallback: () => void, seriesNum: number = 1, isCF: boolean = false): Promise<void> {
...
content = content.slice(0, startIdx) + newFunction + '\n' + content.slice(endIdx);
fs.writeFileSync('src/lib/heatsManager.ts', content, 'utf8');
